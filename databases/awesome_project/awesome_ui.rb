...
# USER INTERFACE
loop do
	puts "Are you working on a character or a campaign? Or to exit type 'exit'"
	character_campaign = gets.chomp
	break if character_campaign == 'exit'
		if character_campaign == "character"
			...
			puts "Are you creating a new campaign or updating a current one? (Type 'new' or 'update')"
			action = gets.chomp
				if action == "new"
					...
					edit = gets.chomp
						...
						if edit == "name"
							...
							puts "Sorry I missed that. Are you updating something for your campaign?"
						end
				else
					puts "Sorry, I didn't catch that. What were you trying to do? (Type 'new' or 'update')"
				end
		else
